...
st.set_page_config(
    page_title="KSOR Program Dashboards",
    layout="wide"
)

st.title("KSOR Program Dashboards")

tab1, tab2 = st.tabs(["RMA Dashboard", "RMS Tracker"])
...
with tab1:
    ...
    st.caption("Active caseload by nationality, Partner Agency, and 30/60/90 termination tracking.")
    ...
    if uploaded_file is not None:
        ...
        active_df["Termination Bucket"] = active_df["Days to Termination"].apply(
            termination_bucket
        )
        ...
        agency_options = ["All"] + sorted(
            active_df["Local Resettlement Agency"].dropna().unique().tolist()
        )
        ...
        with chart_col1:
            ...
            st.bar_chart(
                nationality_chart.set_index("Nationality")
            )
        ...
        with chart_col2:
            ...
            st.bar_chart(
                agency_chart.set_index("Partner Agency")
            )
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        unique_clients["backlog_age_bucket"] = unique_clients["days_pending"].apply(
            backlog_bucket
        )
        ...
        selected_fy = st.sidebar.selectbox("RMS Fiscal Year", available_fys)
